...
function parseSnafu(num: string): number {
    // as numbers in reverse order
    let digitsReversed = num.split('').reverse().map(x => digits[x]);

    // multiply each digit with the corresponding power of 5 and sum them together
    return sum(digitsReversed.map((val, i) => val * Math.pow(5, i)));
}
